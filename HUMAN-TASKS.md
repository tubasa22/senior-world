...
## 🔲 참고 사항 (개선하면 좋은 것, 급하지 않음)

- [ ] 비밀번호 재설정 이메일이 스팸함으로 분류되는 문제 (2026-09-08
  확인). Firebase Auth 기본 발신 주소(`noreply@senior-compass-768f6.firebaseapp.com`)가
  낯선 발신자라 구글메일이 스팸으로 분류함 — 코드/설정 오류는 아니며
  실제로는 정상 발송됨. 방문자들도 스팸함까지 확인 안 하고 "이메일이
  안 온다"고 오해할 수 있으니, 사이트가 커지면 Firebase Console →
  Authentication → 템플릿에서 발신자 주소를 실제 소유 도메인
  (SPF/DKIM 인증 포함)으로 바꾸는 것을 검토할 것.
